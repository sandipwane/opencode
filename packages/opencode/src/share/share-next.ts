...
export namespace ShareNext {
  ...
  export async function create(sessionID: SessionID) {
    ...
    const req = await request()
    const response = await Network.fetch(`${req.baseUrl}${req.api.create}`, {
      method: "POST",
  ...
  async function sync(sessionID: SessionID, data: Data[]) {
    ...
    const timeout = setTimeout(async () => {
      ...
      const req = await request()
      const response = await Network.fetch(`${req.baseUrl}${req.api.sync(share.id)}`, {
        method: "POST",
        headers: { ...req.headers, "Content-Type": "application/json" },
        body: JSON.stringify({
          secret: share.secret,
  ...
  export async function remove(sessionID: SessionID) {
    ...
    const req = await request()
    const response = await Network.fetch(`${req.baseUrl}${req.api.remove(share.id)}`, {
      method: "DELETE",
      headers: { ...req.headers, "Content-Type": "application/json" },
      body: JSON.stringify({
        secret: share.secret,
